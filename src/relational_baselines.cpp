// The two withdrawn relational rules, kept as evaluation baselines.
//
// Neither is sound.  They exist so the evaluation can measure, on the same
// programs, how often each accepts a workflow whose bill moves with a secret.
// The compiler never applies them unless asked to with --relational-rule, and
// the certificate names the rule that produced it.
//
//   sizes   the v3 "billing signature" rule, verbatim apart from moving its
//           types into this file: compares models and variables by local name
//           and requests by estimated size.  Withdrawn 2026-09-25.
//   bounds  the v1 rule: a secret-guarded branch is accepted when its arms
//           have equal certified total bounds.  Withdrawn 2026-09-17.
...
RelationalResult analyzeWithBoundsRule(const Program& program, const SemanticResult& semantic) {
    ...
    for (const auto& workflowPointer : program.workflows) {
        ...
        const std::string workflow = workflowPointer->name;
        std::vector<const Block*> pending{&workflowPointer->statements};
        while (!pending.empty()) {
            const Block* block = pending.back();
            pending.pop_back();
            for (const auto& statement : *block) {
                if (!statement) {
                    continue;
                ...
                if (statement->kind() == StmtKind::Retry) {
                    pending.push_back(&static_cast<const RetryStmt&>(*statement).body);
                    continue;
                }
                if (statement->kind() != StmtKind::If) {
                    continue;
                ...
                const auto& branch = static_cast<const IfStmt&>(*statement);
                pending.push_back(&branch.thenBranch);
                pending.push_back(&branch.elseBranch);
                // v1 tested the guard label with the enclosing context joined.
                const auto guard = semantic.guardLabels.find(&branch);
                if (guard == semantic.guardLabels.end() || !guard->second.isSecret()) {
                    continue;
                }
                const CostBound thenBound = boundOfBlock(branch.thenBranch, semantic);
                const CostBound elseBound = boundOfBlock(branch.elseBranch, semantic);
                RelationalObligation obligation;
                obligation.workflow = workflow;
                obligation.guard = conditionToString(branch.condition);
                obligation.location = branch.location;
                obligation.discharged = thenBound.total() == elseBound.total();
                obligation.detail = "arm bounds " + std::to_string(thenBound.total()) + " and " +
                                    std::to_string(elseBound.total());
                if (!obligation.discharged) {
                    result.diagnostics.error("E236", branch.location,
                                             "[bounds rule] the arms of this secret-guarded branch "
                                             "have different certified bounds (" + obligation.detail + ")");
                }
